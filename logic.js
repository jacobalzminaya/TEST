...
const STRENGTH_CONFIG = {
  LEVELS: {
    1: { name: 'DÉBIL', icon: '▰', color: '#888', desc: 'Doji puro' },
...
function resetConfirmationPanel() {
  ...
  if (gBtn) gBtn.className = 'color-toggle-btn';
  if (rBtn) rBtn.className = 'color-toggle-btn';
  buildPickerGrid('G');
  updatePickerConfirmBtn();
  ...
  document.getElementById('correction-notice').classList.remove('active');
  document.getElementById('correction-options').style.display = 'block';
  document.getElementById('color-buttons').style.display = 'none'; // legacy hidden
  document.getElementById('step-confirm').style.display = 'block';
...
function update() {
  ...
  // ========== PRIORIDAD 1: VOLUME PROFILE ==========
    ...
    if (nearPOC && currentCandle.strength >= 3) {
      ...
      if (currentCandle.current === 'G') {
        ...
        logicText = `🎯 REVERSIÓN EN POC (${vp.poc.toFixed(1)}%): Vela verde fuerza ${currentCandle.strength}/3. Soporte fuerte.`;
      } else {
        finalSignal = 'sell'; signalClass = 'signal-strong-reversal'; signalText = '🎯 VENDER';
        logicText = `🎯 REVERSIÓN EN POC (${vp.poc.toFixed(1)}%): Vela roja fuerza ${currentCandle.strength}/3. Resistencia fuerte.`;
      }
      ...
      if (currentCandle.current === 'G') {
        ...
        if (price >= highZone) {
          ...
          // Zona baja del VA: tiene recorrido hasta POC y VAH — compra confirmada
          finalSignal = 'buy'; signalClass = 'signal-buy'; signalText = '⬆️ COMPRAR';
          logicText = `📊 DENTRO VA zona baja (${price.toFixed(1)}%) + verde fuerza ${currentCandle.strength}. Recorrido hacia POC (${vp.poc.toFixed(1)}%) y VAH (${vp.vah.toFixed(1)}%).`;
        }
      } else {
        ...
        if (price <= lowZone) {
          ...
          finalSignal = 'sell'; signalClass = 'signal-sell'; signalText = '⬇️ VENDER';
          logicText = `📊 DENTRO VA: Momentum bajista hacia VAL (${vp.val.toFixed(1)}%).`;
        }
      }
    }
  }
  ...
  // ========== SAFETY NET: EXTREMO anula cualquier señal residual ==========
  // Si por algún path el extremo está activo pero la señal no se bloqueó,
  // forzar el bloqueo aquí como última línea de defensa.
  if (extremeCondition) {
  ...
  mainSignal.innerHTML = signalText + 
    ...
    '<div class="extreme-signal-badge">🚨 EXTREMO</div>' +
    '<div class="cmd-signal-badge">🎯 FASE CMD</div>' +
    '<div class="candle-trap-badge">🪤 POSIBLE TRAMPA</div>' +
    '<div class="weak-buy-badge">⚡ CONFIANZA MEDIA</div>' +
    '<div class="caution-badge">🛑 PRECAUCIÓN</div>';

  // ===== TEXTO DE VOZ: diferente según el tipo real de señal =====
  // Se guarda en data-voice para que el index.html pueda leerlo con TTS
  const voiceTexts = {
    ...
    'signal-buy-weak':       '⚠️ Posible compra, confianza media, verificar antes de entrar',
    'signal-sell-weak':      '⚠️ Posible venta, confianza media, verificar antes de entrar',
    'signal-caution':        '🛑 Precaución, zona límite, no operar todavía',
    ...
    'signal-conflict':       '⚠️ Esperar, señales contradictorias',
    'signal-manipulation':   '🎭 Esperar, manipulación detectada, no operar',
    'signal-trap':           '🪤 Posible trampa, no operar',
    'signal-extreme':        '🚫 Extremo, esperar confirmación',
    'signal-wait':           'Esperando señal',
  };
  mainSignal.setAttribute('data-voice', voiceTexts[signalClass] || signalText.replace(/[⬆️⬇️⚠️🔄🎯🔥📥🏦✅🚫🪤🎭]/g, '').trim());
...
// Estado del picker
let pickerColor    = null;   // 'G' o 'R'
let pickerStrength = null;   // valor de strength del tipo seleccionado
let pickerTypeId   = null;   // id único del tipo (para distinguir doji vs longleg, ambos strength=1)
...
function buildPickerGrid(color) {
  ...
  grid.innerHTML = CANDLE_TYPES.map(type => {
    ...
    const badgeG = buildPickerBadge(type, 'G');
    const badgeR = buildPickerBadge(type, 'R');
    const badge  = selG ? badgeG : selR ? badgeR : badgeG;
    ...
    return `<div class="candle-type-row ${rowClass}" id="crow-${type.id}">
      <div class="ctr-name">
        ...
        <div class="ctr-badge">${badge}</div>
      </div>
      <button class="ctr-btn ctr-g ${selG ? 'ctr-active-g' : ''}"
              onclick="pickerDirectAdd('${type.id}', ${type.strength}, 'G')"
              title="${type.label} verde — click para añadir">
        ${buildCandleSVG(type, 'G')}
        <div class="ctr-btn-label">🟢</div>
      </button>
      <button class="ctr-btn ctr-r ${selR ? 'ctr-active-r' : ''}"
              onclick="pickerDirectAdd('${type.id}', ${type.strength}, 'R')"
              title="${type.label} rojo — click para añadir">
...
// Un click en 🟢 o 🔴 → selecciona tipo + color → añade directo
function pickerDirectAdd(typeId, strength, color) {
  selectCandleType(typeId, strength);
  setCandlePickColor(color);
  addCandleFromPicker();
}
...
function setCandlePickColor(color) {
  ...
  const rBtn = document.getElementById('pick-color-red');
  if (gBtn && rBtn) {
    gBtn.className = 'color-toggle-btn' + (color === 'G' ? ' active-green' : '');
    rBtn.className = 'color-toggle-btn' + (color === 'R' ? ' active-red' : '');
  }
...
function updatePickerConfirmBtn() {
  ...
  if (pickerColor && pickerTypeId) {
    btn.classList.add('ready');
    btn.classList.remove('green-ready', 'red-ready');
    const type = CANDLE_TYPES.find(t => t.id === pickerTypeId);
    const typeName = type ? type.label : '';
    if (pickerColor === 'G') {
      btn.classList.add('green-ready');
      btn.textContent = `🟢 Añadir VERDE — ${typeName}`;
    } else {
      btn.classList.add('red-ready');
      btn.textContent = `🔴 Añadir ROJO — ${typeName}`;
    }
  } else if (pickerColor && !pickerTypeId) {
    btn.classList.remove('ready', 'green-ready', 'red-ready');
    btn.textContent = '← Selecciona una vela';
  } else {
    btn.classList.remove('ready', 'green-ready', 'red-ready');
...
function addCandleFromPicker() {
  ...
  addCandle(pickerColor);
  // Visual flash feedback on the confirm button
  const btn = document.getElementById('add-candle-confirm-btn');
  if (btn) {
...
// Inicializar el picker cuando carga la página
