...
function update() {
  ...
  var _insuficienteDatos = history.length < 3;
  ...
  if (_insuficienteDatos) {
    finalSignal  = 'wait';
    signalClass  = 'signal-wait';
    signalText   = '⏳ ACUMULANDO DATOS';
    logicText    = 'Se necesitan al menos 3 velas para calcular tendencia. Llevas ' + history.length + (history.length === 1 ? ' vela.' : ' velas.');
...
function fillProjBox(side, proj) {
  ...
  // Secuencia con velas SVG — ultima es prominente (vela siguiente)
  ...
  if (pctEl)  { pctEl.textContent = pct + '%'; pctEl.style.color = pct >= 70 ? barColor : '#555'; }

  // Descripcion
  if (confEl) { confEl.textContent = data.desc || ''; }

  // Resaltar box si es la señal dominante
  if (boxEl) {
    var borderColor = isBuy ? 'rgba(0,255,136,0.6)' : isSell ? 'rgba(255,68,68,0.6)' : 'rgba(255,215,0,0.3)';
    boxEl.style.borderTopColor = borderColor;
  }
}
...
window.showPPProjection = function(color, strength, clientX, clientY) {
  ...
  if (projectionMode === 'current') {
    ...
    // La proyeccion muestra la vela DESPUES de registrar esta
