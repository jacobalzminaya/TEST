...
function update() {
  ...
  document.getElementById('history').innerHTML = history.map((h, i) => {
    ...
    return `<div class="candle candle-${h.toLowerCase()} ${isTrap?'candle-trap':''} ${isNew&&!isTrap?'candle-new':''} ${extraClass}" onclick="openEditPanel(${i})" title="Vela #${i+1}: ${h}, Fuerza: ${s}/4 (${strengthLabel}), Origen: ${origin}">${h}${indicator}${manualMark}</div>`;
  }).join('');
}
...
initCandlePicker();
initFocusMode();
console.log('🚀 Detector CMD iniciado (refactorizado).');
