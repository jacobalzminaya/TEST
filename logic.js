...
function resetConfirmationPanel() {
  ...
  // El preview de vela se actualiza dentro de updateSuggestionPanel() arriba
}
...
function buildPickerGrid(color) {
  ...
  grid.innerHTML = CANDLE_TYPES.map(type => {
    ...
    // Configuración de fuerza — color y nombre del nivel
    ...
    // Fondo sutil por nivel solo cuando la fila NO está seleccionada
    const strBg = rowClass ? '' :
                  type.strength >= 4   ? 'rgba(0,255,204,0.06)'   :
                  type.strength >= 3   ? 'rgba(0,255,136,0.05)'   :
                  type.strength >= 2.5 ? 'rgba(255,204,0,0.05)'   :
                  type.strength >= 2   ? 'rgba(255,255,255,0.03)' :
                                         'rgba(255,255,255,0.01)';

    // Badge de fuerza — el núcleo de esta mejora
    const strengthBadge = `<span style="
        display:inline-flex;align-items:center;gap:3px;
        padding:1px 5px;border-radius:3px;
        font-size:0.65em;font-weight:bold;
        color:${strColor};
        background:${strColor}18;
        border:1px solid ${strColor}44;
      ">F${type.strength} ${strName}</span>`;

    // Marca "← sugerida" si el sistema sugirió esta fuerza exacta
    const isSuggested = typeof autoSuggestedStrength !== 'undefined' &&
                        type.strength === autoSuggestedStrength;
    const suggestedMark = isSuggested
      ? `<span style="
          padding:1px 5px;border-radius:3px;
          font-size:0.6em;font-weight:bold;
          color:#00ffaa;background:#00ffaa14;
          border:1px solid #00ffaa44;
          margin-left:3px;
        ">✓ sugerida</span>`
      : '';

    return `<div class="candle-type-row ${rowClass}" id="crow-${type.id}" style="background:${strBg}">
      <div class="ctr-name">
        <div class="ctr-label" style="display:flex;align-items:center;flex-wrap:wrap;gap:3px;">
          ${type.label}${suggestedMark}
        </div>
        <div class="ctr-sub">${type.sublabel}</div>
        <div style="display:flex;align-items:center;gap:4px;margin-top:2px;flex-wrap:wrap;">
          ${strengthBadge}
          <span class="ctr-badge">${badge}</span>
        </div>
      </div>
      <button class="ctr-btn ctr-g ${selG ? 'ctr-active-g' : ''}"
              onclick="pickerDirectAdd('${type.id}', ${type.strength}, 'G')"
              title="${type.label} · F${type.strength} ${strName} · Verde">
      ...
      <button class="ctr-btn ctr-r ${selR ? 'ctr-active-r' : ''}"
              ...
              title="${type.label} · F${type.strength} ${strName} · Roja">
