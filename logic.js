...
function update() {
  ...
  // ========== PRIORIDAD 1: VOLUME PROFILE ==========
    ...
    if (nearPOC && currentCandle.strength >= 3) {
      ...
      // FIX 3: En POC con vela débil → ESPERAR siempre, independientemente del cruce.
      // El POC es el nivel de máximo volumen: el precio tiende a oscilar en él.
      // Una vela débil en el POC no confirma ni ruptura ni rebote.
      showVP = true;
...
const SIZE_LEVELS = [
  { strength: 1,   label: 'Doji',       sublabel: 'Línea plana',      bodyH: 0.04, wickH: 0.46 },
  { strength: 1.5, label: 'Pequeña',    sublabel: 'Cuerpo chico',     bodyH: 0.16, wickH: 0.36 },
  { strength: 2,   label: 'Normal',     sublabel: 'Cuerpo medio',     bodyH: 0.32, wickH: 0.26 },
  { strength: 2.5, label: 'Grande',     sublabel: 'Cuerpo dominante', bodyH: 0.50, wickH: 0.14 },
  { strength: 3,   label: 'Muy grande', sublabel: 'Mechas mínimas',   bodyH: 0.68, wickH: 0.06 },
  { strength: 4,   label: 'Marubozu',   sublabel: 'Sin mechas',       bodyH: 0.88, wickH: 0.01 },
];

// Construye el SVG de cada celda del picker (vela proporcional al tamaño)
function buildSizeSVG(level, color) {
  const W = 32, H = 54;
  const cx = W / 2;
  const bW = 10;
  ...
  const totalUsed = level.bodyH * H + level.wickH * 2 * H;
  const margin    = (H - totalUsed) / 2;
  const wickTop   = margin;
  const bodyTop   = margin + level.wickH * H;
  const bodyH     = level.bodyH * H;
  const wickBot   = bodyTop + bodyH;
  const wickBotEnd= wickBot + level.wickH * H;

  const filterId  = `glow-sz-${level.strength}-${color}`.replace('.','_');
  const filterDef = `<filter id="${filterId}" x="-80%" y="-80%" width="260%" height="260%">
    <feGaussianBlur stdDeviation="1.5" result="b"/>
    <feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge>
  </filter>`;

  const mkWick = (y1, y2) => y2 - y1 > 0.5
    ? `<line x1="${cx}" y1="${y1.toFixed(1)}" x2="${cx}" y2="${y2.toFixed(1)}" stroke="${stroke}" stroke-width="1.5" stroke-linecap="round"/>`
    : '';
  const mkBody = (y, h) =>
    `<rect x="${(cx - bW/2).toFixed(1)}" y="${y.toFixed(1)}" width="${bW}" height="${Math.max(h,2).toFixed(1)}" fill="${fill}" rx="2" filter="url(#${filterId})" opacity="0.93"/>`;

  const body = mkWick(wickTop, bodyTop) + mkBody(bodyTop, bodyH) + mkWick(wickBot, wickBotEnd);
  return `<svg viewBox="0 0 ${W} ${H}" xmlns="http://www.w3.org/2000/svg" style="display:block;width:${W}px;height:${H}px"><defs>${filterDef}</defs>${body}</svg>`;
}

// Renderiza las 2 filas del picker en el contenedor
function buildSizePicker() {
  const container = document.getElementById('size-picker-container');
  if (!container) return;

  const strColors = { 1:'#888', 1.5:'#aaa', 2:'#ffcc00', 2.5:'#ffcc00', 3:'#00ff88', 4:'#00ffcc' };

  const makeRow = (color, emoji) => `
    <div class="sp-row sp-row-${color === 'G' ? 'green' : 'red'}">
      <div class="sp-row-label">${emoji}</div>
      ${SIZE_LEVELS.map(level => `
        <button class="sp-cell" onclick="quickAddCandle('${color}', ${level.strength})"
                title="${emoji} ${level.label} — Fuerza ${level.strength}">
          <div class="sp-svg-wrap">${buildSizeSVG(level, color)}</div>
          <div class="sp-cell-label" style="color:${strColors[level.strength]}">${level.label}</div>
          <div class="sp-cell-str" style="color:${strColors[level.strength]}">F·${level.strength}${level.strength === 4 ? '🔥' : ''}</div>
        </button>
      `).join('')}
    </div>`;

  container.innerHTML = makeRow('G', '🟢') + makeRow('R', '🔴');
}

// Añade la vela al instante — sin confirmación, sin pasos extra
function quickAddCandle(color, strength) {
  // Establecer la fuerza directamente
  confirmedStrength = strength;
  strengthOrigin    = 'corrected';

  // Feedback visual: flash en la celda clickeada
  const cells = document.querySelectorAll('.sp-cell');
  cells.forEach(c => c.classList.remove('sp-cell-flash'));

  // Añadir la vela
  addCandle(color);

  // Flash de confirmación en la fila correspondiente
  const rowEl = document.querySelector(`.sp-row-${color === 'G' ? 'green' : 'red'}`);
  if (rowEl) {
    rowEl.classList.add('sp-flash');
    setTimeout(() => rowEl.classList.remove('sp-flash'), 220);
  }
}

// Inicializar el nuevo picker
function initCandlePicker() {
  buildSizePicker();
}

// ── Funciones legacy mantenidas para compatibilidad con Boston overlay ────────
// (Boston usa pickerDirectAdd → estas funciones siguen funcionando igual)
...
function buildPickerGrid(color) {
  // Delegado al nuevo picker — no hace nada en el panel principal
  // pero Boston overlay sigue llamando a esta función
  const grid = document.getElementById('candle-picker-grid');
  if (!grid) return;
  // Mantener Boston grid funcional con el sistema antiguo
  grid.innerHTML = CANDLE_TYPES.map(type => {
    ...
    return `<div class="candle-type-row ${rowClass}" id="crow-${type.id}">
      <div class="ctr-name">
...
function pickerDirectAdd(typeId, strength, color) {
  quickAddCandle(color, strength);
}

function selectCandleType(typeId, strength) {
  pickerTypeId   = typeId;
  pickerStrength = strength;
  correctStrength(strength);
  if (pickerColor) buildPickerGrid(pickerColor);
  updatePickerConfirmBtn();
}

function setCandlePickColor(color) {
  pickerColor = color;
  const gBtn = document.getElementById('pick-color-green');
  const rBtn = document.getElementById('pick-color-red');
  if (gBtn) gBtn.className = 'color-toggle-btn' + (color === 'G' ? ' active-green' : '');
  if (rBtn) rBtn.className = 'color-toggle-btn' + (color === 'R' ? ' active-red' : '');
  buildPickerGrid(color);
  updatePickerConfirmBtn();
}

function updatePickerConfirmBtn() {
  const btn = document.getElementById('add-candle-confirm-btn');
  if (!btn) return;
  btn.classList.remove('ready', 'green-ready', 'red-ready');
  btn.textContent = '← Usa las filas de arriba';
}

function addCandleFromPicker() {
  if (!pickerColor || !pickerStrength) return;
  quickAddCandle(pickerColor, pickerStrength);
  pickerColor = null; pickerStrength = null; pickerTypeId = null;
}
...
// ===== PRICE PICKER — selector por precio real =====
//
// El usuario define un rango (min-max), hace click en la barra donde cerró
// la vela, y el sistema calcula automáticamente color y fuerza.
//
// Estado del picker de precio
let ppRefPrice  = null;  // precio de cierre de la vela anterior (referencia)
let ppMinPrice  = null;  // límite inferior del rango actual
let ppMaxPrice  = null;  // límite superior del rango actual

// Zonas de fuerza — mapeo de % del rango recorrido → fuerza
const PP_ZONES = [
  { minPct: 85, strength: 4,   label: 'Marubozu',    color: '#00ffcc' },
  { minPct: 65, strength: 3,   label: 'Muy grande',  color: '#00ff88' },
  { minPct: 45, strength: 2.5, label: 'Grande',      color: '#88ff44' },
  { minPct: 25, strength: 2,   label: 'Normal',      color: '#ffcc00' },
  { minPct:  8, strength: 1.5, label: 'Pequeña',     color: '#ff8800' },
  { minPct:  0, strength: 1,   label: 'Doji',        color: '#888888' },
];

// Calcula la fuerza según porcentaje del rango recorrido
function ppStrengthFromPct(pct) {
  for (const z of PP_ZONES) {
    if (pct >= z.minPct) return z;
  }
  return PP_ZONES[PP_ZONES.length - 1];
}

// Zonas con colores para modo normal y hover (verde y rojo)
const PP_ZONE_DEFS = [
  { pct:[85,100], str:4,   name:'Marubozu',   gN:'rgba(0,220,100,0.22)', rN:'rgba(220,50,50,0.22)', gH:'rgba(0,220,100,0.45)', rH:'rgba(220,50,50,0.45)' },
  { pct:[65, 85], str:3,   name:'Muy grande', gN:'rgba(0,220,100,0.15)', rN:'rgba(220,50,50,0.15)', gH:'rgba(0,220,100,0.34)', rH:'rgba(220,50,50,0.34)' },
  { pct:[45, 65], str:2.5, name:'Grande',     gN:'rgba(0,220,100,0.09)', rN:'rgba(220,50,50,0.09)', gH:'rgba(0,220,100,0.24)', rH:'rgba(220,50,50,0.24)' },
  { pct:[25, 45], str:2,   name:'Normal',     gN:'rgba(0,220,100,0.05)', rN:'rgba(220,50,50,0.05)', gH:'rgba(0,220,100,0.16)', rH:'rgba(220,50,50,0.16)' },
  { pct:[ 8, 25], str:1.5, name:'Pequeña',    gN:'rgba(0,220,100,0.03)', rN:'rgba(220,50,50,0.03)', gH:'rgba(0,220,100,0.11)', rH:'rgba(220,50,50,0.11)' },
  { pct:[ 0,  8], str:1,   name:'Doji',       gN:'rgba(150,150,150,0.05)',rN:'rgba(150,150,150,0.05)',gH:'rgba(150,150,150,0.18)',rH:'rgba(150,150,150,0.18)' },
];

function ppZoneIdxFromPct(pct) {
  if (pct >= 85) return 0;
  if (pct >= 65) return 1;
  if (pct >= 45) return 2;
  if (pct >= 25) return 3;
...
// Dibuja la barra con zonas visibles, precios legibles y cursor pointer
function ppRenderBar() {
  const bar = document.getElementById('pp-bar');
  if (!bar) return;

  const min = ppMinPrice, max = ppMaxPrice, ref = ppRefPrice;

  if (min === null || max === null || max <= min) {
    bar.innerHTML = '<div style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;color:#444;font-size:0.72em;">Introduce el rango mín y máx</div>';
    return;
  }
  ...
  // Zonas de fondo — todas en verde por defecto (se colorean en hover)
  PP_ZONE_DEFS.forEach((z, i) => {
    const top = (100 - z.pct[1]) + '%';
    const h   = (z.pct[1] - z.pct[0]) + '%';
    const pHi = (min + z.pct[1] / 100 * range).toFixed(2);
    const pLo = (min + z.pct[0] / 100 * range).toFixed(2);
    const pMid= (min + (z.pct[0] + z.pct[1]) / 2 / 100 * range).toFixed(2);
    html += `<div class="ppz" id="ppz${i}" data-zi="${i}" style="
      position:absolute;left:0;right:0;top:${top};height:${h};
      background:${z.gN};
      display:flex;align-items:center;
      border-bottom:1px solid rgba(255,255,255,0.05);
      transition:background .08s;
    ">
      <span style="position:absolute;left:8px;top:50%;transform:translateY(-50%);font-size:10px;font-weight:500;color:rgba(0,220,100,0.8);pointer-events:none;white-space:nowrap;">
        ${z.name} · F${z.str}
      </span>
      <span style="position:absolute;right:8px;top:50%;transform:translateY(-50%);font-size:10px;color:rgba(255,255,255,0.35);pointer-events:none;">
        ${pHi}
      </span>
    </div>`;
  });

  // Separadores de precio cada 25%
  [25, 50, 75].forEach(p => {
    const price = (min + p / 100 * range).toFixed(2);
    html += `<div style="position:absolute;left:0;right:0;top:${100-p}%;height:1px;background:rgba(255,255,255,0.08);pointer-events:none;z-index:2;">
      <span style="position:absolute;left:50%;transform:translateX(-50%);bottom:2px;font-size:10px;color:rgba(255,255,255,0.4);">${price}</span>
    </div>`;
  });

  // Extremos del rango
  html += `<div style="position:absolute;top:3px;left:8px;font-size:10px;color:rgba(255,255,255,0.5);pointer-events:none;">${max.toFixed(2)} ↑ máx</div>`;
  html += `<div style="position:absolute;bottom:3px;left:8px;font-size:10px;color:rgba(255,255,255,0.5);pointer-events:none;">${min.toFixed(2)} ↓ mín</div>`;

  // Línea de referencia azul
  if (ref !== null && ref >= min && ref <= max) {
    const refTop = ((max - ref) / range) * 100;
    html += `<div style="position:absolute;left:0;right:0;top:${refTop}%;height:2px;background:#4499ff;z-index:4;pointer-events:none;">
      <span style="position:absolute;left:8px;bottom:3px;font-size:10px;color:#4499ff;font-weight:bold;">ref ${ref.toFixed(2)}</span>
    </div>`;
  }

  // Línea de hover (oculta inicialmente)
  html += `<div id="pp-hline" style="position:absolute;left:0;right:0;height:2px;display:none;pointer-events:none;z-index:6;transition:top .04s;">
    <span id="pp-hprice" style="position:absolute;right:8px;top:-16px;font-size:11px;font-weight:bold;padding:1px 6px;border-radius:3px;"></span>
    <span id="pp-hdir"   style="position:absolute;left:8px;top:-16px;font-size:11px;font-weight:bold;padding:1px 6px;border-radius:3px;"></span>
  </div>`;
  ...
  // Eventos hover
  bar.onmousemove = ppBarHover;
  bar.onmouseleave = ppBarLeave;
}

// Hover: resalta zona y muestra línea con precio y dirección
function ppBarHover(e) {
  const bar  = document.getElementById('pp-bar');
  if (!bar || ppMinPrice === null) return;
  const rect = bar.getBoundingClientRect();
  const yPct = (e.clientY - rect.top) / rect.height;
  const price = Math.max(ppMinPrice, Math.min(ppMaxPrice, ppMaxPrice - yPct * (ppMaxPrice - ppMinPrice)));
  const isGreen = ppRefPrice === null ? true : price >= ppRefPrice;
  const range = ppMaxPrice - ppMinPrice;
  const pct = ppRefPrice !== null ? Math.min(100, Math.abs(price - ppRefPrice) / range * 100) : 50;
  const zi = ppZoneIdxFromPct(pct);
  const z = PP_ZONE_DEFS[zi];
  const gColor = '#00cc66', rColor = '#ff4444';
  const activeColor = isGreen ? gColor : rColor;

  // Actualizar fondo de zonas
  PP_ZONE_DEFS.forEach((zd, i) => {
    const el = document.getElementById('ppz' + i);
    if (!el) return;
    const bg = i === zi ? (isGreen ? zd.gH : zd.rH) : (isGreen ? zd.gN : zd.rN);
    el.style.background = bg;
    const nameEl = el.querySelector('span');
    if (nameEl) nameEl.style.color = i === zi ? activeColor : (isGreen ? 'rgba(0,220,100,0.7)' : 'rgba(220,80,80,0.7)');
  });

  // Línea de hover
  const yPx = e.clientY - rect.top;
  const hl = document.getElementById('pp-hline');
  const hp = document.getElementById('pp-hprice');
  const hd = document.getElementById('pp-hdir');
  if (!hl || !hp || !hd) return;
  hl.style.display = 'block';
  hl.style.top = yPx + 'px';
  hl.style.background = activeColor;
  hp.textContent = price.toFixed(2);
  hp.style.background = isGreen ? 'rgba(0,180,70,0.2)' : 'rgba(200,40,40,0.2)';
  hp.style.color = activeColor;
  hd.textContent = (isGreen ? '▲ VERDE' : '▼ ROJA') + ' · ' + z.name + ' · F' + z.str;
  hd.style.background = isGreen ? 'rgba(0,180,70,0.2)' : 'rgba(200,40,40,0.2)';
  hd.style.color = activeColor;
}

// Al salir del hover: restaurar colores neutros
function ppBarLeave() {
  PP_ZONE_DEFS.forEach((z, i) => {
    const el = document.getElementById('ppz' + i);
    if (el) {
      el.style.background = z.gN;
      const nameEl = el.querySelector('span');
      if (nameEl) nameEl.style.color = 'rgba(0,220,100,0.7)';
    }
  });
  const hl = document.getElementById('pp-hline');
...
// Llamado cuando el usuario cambia el rango manualmente
function ppRebuild() {

// Llamado cuando el usuario cambia el rango manualmente
function ppRebuild() {
  const minEl = document.getElementById('pp-min');
  const maxEl = document.getElementById('pp-max');
  ...
  if (!isNaN(minV) && !isNaN(maxV) && maxV > minV) {
    ...
    if (res) {
      res.style.background = '#0d0d0d';
      res.style.borderColor = '#222';
      res.style.color = '#555';
      res.textContent = 'Haz click donde cerró la vela';
    }
  }
}

// Llamado cuando el usuario hace click en la barra
function ppBarClick(e) {
  if (ppMinPrice === null || ppMaxPrice === null) return;
  if (ppMaxPrice <= ppMinPrice) return;
  const bar  = document.getElementById('pp-bar');
  if (!bar) return;
  const rect = bar.getBoundingClientRect();
  const yPct    = (e.clientY - rect.top) / rect.height;
  const price   = ppMaxPrice - yPct * (ppMaxPrice - ppMinPrice);
  const clampedPrice = Math.max(ppMinPrice, Math.min(ppMaxPrice, price));
  ppProcessPrice(clampedPrice);
}

// Llamado cuando el usuario teclea un precio y pulsa Enter/OK
function ppManualEnter() {
  const input = document.getElementById('pp-manual');
  if (!input) return;
  const price = parseFloat(input.value);
  if (isNaN(price)) return;
  input.value = '';

  // Si no hay rango configurado, centrar el rango alrededor del precio
  if (ppMinPrice === null || ppMaxPrice === null) {
    const spread = 2;
    ppMinPrice = price - spread;
    ppMaxPrice = price + spread;
    const minEl = document.getElementById('pp-min');
    const maxEl = document.getElementById('pp-max');
    if (minEl) minEl.value = ppMinPrice.toFixed(2);
    if (maxEl) maxEl.value = ppMaxPrice.toFixed(2);
    ppRenderBar();
  }

  ppProcessPrice(price);
}

// Lógica central: dado un precio de cierre, determinar color y fuerza
function ppProcessPrice(closePrice) {
  const range = ppMaxPrice - ppMinPrice;

  // Si no hay referencia previa, el precio inicial define la ref y pide la siguiente
  if (ppRefPrice === null) {
    ppRefPrice = closePrice;
    const refLabel = document.getElementById('pp-ref-label');
    if (refLabel) refLabel.textContent = `ref: ${closePrice.toFixed(2)}`;
    const res = document.getElementById('pp-result');
    if (res) {
      res.style.background = '#0d1a0d';
      res.style.borderColor = '#225522';
      res.style.color = '#4488aa';
      res.innerHTML = `Precio de apertura registrado: <b>${closePrice.toFixed(2)}</b> — ahora haz click donde cerró la vela`;
    }
    ppRenderBar();
    return;
  }

  // Calcular movimiento desde la referencia
  const move    = closePrice - ppRefPrice;
  const isGreen = move >= 0;
  const recorrido = Math.abs(move);
  const pct     = Math.min(100, (recorrido / range) * 100);

  // Determinar zona de fuerza
  const zone = ppStrengthFromPct(pct);

  // Feedback visual en el resultado
  const res = document.getElementById('pp-result');
  if (res) {
    const arrow  = isGreen ? '▲' : '▼';
    const dirText = isGreen ? 'VERDE' : 'ROJA';
    const dirColor = isGreen ? '#00cc66' : '#ff4444';
    res.style.background  = isGreen ? 'rgba(0,180,80,0.12)' : 'rgba(220,50,50,0.12)';
    res.style.borderColor = isGreen ? 'rgba(0,180,80,0.4)'  : 'rgba(220,50,50,0.4)';
    res.style.color       = dirColor;
    res.innerHTML = `
      ${arrow} Vela <b>${dirText}</b>
      &nbsp;·&nbsp; Cierre <b>${closePrice.toFixed(2)}</b>
      &nbsp;·&nbsp; Mov. <b>${move >= 0 ? '+' : ''}${move.toFixed(2)}</b>
      &nbsp;·&nbsp; <b>${pct.toFixed(0)}%</b> del rango
      &nbsp;·&nbsp; F<b>${zone.strength}</b> ${zone.label}
    `;
  }

  // Añadir la vela al sistema con la fuerza calculada
  confirmedStrength = zone.strength;
  strengthOrigin    = 'corrected';
  addCandle(isGreen ? 'G' : 'R');

  // Flash en la barra
  const bar = document.getElementById('pp-bar');
  if (bar) {
    bar.style.borderColor = isGreen ? '#00ff88' : '#ff4444';
    setTimeout(() => { if (bar) bar.style.borderColor = '#2a2a2a'; }, 300);
  }

  // Actualizar referencia al precio de cierre actual
  ppRefPrice = closePrice;
  const refLabel = document.getElementById('pp-ref-label');
  if (refLabel) refLabel.textContent = `ref: ${closePrice.toFixed(2)}`;

  // Centrar el rango alrededor del nuevo precio para la siguiente vela
  const halfRange = (ppMaxPrice - ppMinPrice) / 2;
  ppMinPrice = closePrice - halfRange;
  ppMaxPrice = closePrice + halfRange;
  const minEl = document.getElementById('pp-min');
  const maxEl = document.getElementById('pp-max');
  if (minEl) minEl.value = ppMinPrice.toFixed(2);
  if (maxEl) maxEl.value = ppMaxPrice.toFixed(2);
...
// Toggle entre modo precio y modo visual
function setPrickerMode(mode) {
  const pricePanel  = document.getElementById('price-picker-panel');
  const visualPanel = document.getElementById('visual-picker-panel');
  const priceBtn    = document.getElementById('mode-price-btn');
  const visualBtn   = document.getElementById('mode-visual-btn');

  if (mode === 'price') {
    if (pricePanel)  pricePanel.style.display  = 'block';
    if (visualPanel) visualPanel.style.display = 'none';
    if (priceBtn)  { priceBtn.style.background  = '#00d4ff22'; priceBtn.style.color  = '#00d4ff'; }
    if (visualBtn) { visualBtn.style.background = 'transparent'; visualBtn.style.color = '#555'; }
  } else {
    if (pricePanel)  pricePanel.style.display  = 'none';
    if (visualPanel) visualPanel.style.display = 'block';
    if (visualBtn) { visualBtn.style.background  = '#00d4ff22'; visualBtn.style.color  = '#00d4ff'; }
    if (priceBtn)  { priceBtn.style.background = 'transparent'; priceBtn.style.color = '#555'; }
    // Asegurar que el size picker esté renderizado
    if (typeof buildSizePicker === 'function') buildSizePicker();
  }
}
