...
function update() {
  ...
  document.getElementById('history').innerHTML = history.map((h, i) => {
    ...
    return `<div class="candle candle-${h.toLowerCase()} ${isTrap?'candle-trap':''} ${isNew&&!isTrap?'candle-new':''} ${extraClass}" data-hoverIdx="${i}" onclick="openEditPanel(${i})" title="Vela #${i+1}: ${h}, Fuerza: ${s}/4 (${strengthLabel}), Origen: ${origin}">${h}${indicator}${manualMark}</div>`;
  }).join('');
}
...
// ===== MODO DE PROYECCIÓN (Vela Actual / Vela Siguiente) =====

let projectionMode = 'current'; // 'current' | 'next'
let hoverProjPinned = false;

function toggleProjectionMode() {
  projectionMode = projectionMode === 'current' ? 'next' : 'current';
  const sw = document.getElementById('proj-mode-switch');
  const badge = document.getElementById('hover-proj-mode-badge');
  if (projectionMode === 'next') {
    sw.classList.add('mode-next');
    if (badge) badge.textContent = '⚡ VELA SIGUIENTE';
  } else {
    sw.classList.remove('mode-next');
    if (badge) badge.textContent = '📍 VELA ACTUAL';
  }
  // Si hay panel fijado, recalcular con nuevo modo
  if (hoverProjPinned) {
    const panel = document.getElementById('hover-proj-panel');
    const candleInfo = document.getElementById('hover-proj-candle-info');
    if (panel && panel.style.display !== 'none') {
      const idx = parseInt(panel.dataset.idx || '-1');
...
/**
 * Calcula la proyección para una vela del historial dada.
 * Modo 'current': proyecta qué pasará después de esa vela (vela actual del análisis).
 * Modo 'next':    proyecta qué pasará después de la vela SIGUIENTE a la seleccionada,
 *                 anticipando la tendencia futura.
 *
 * La lógica interna de análisis (analyzePattern, extremeCondition, cmdState, MAs) NO cambia.
 * Solo varía la ventana de historial que se pasa como contexto.
 */
function computeHoverProjection(idx) {
  ...
  let effectiveHistory = history;
  if (manualTrapIndex !== null) {
    effectiveHistory = history.filter((_, i) => i !== manualTrapIndex);
  }

  let windowEnd; // índice exclusivo en history[] que define el contexto
  if (projectionMode === 'current') {
    // Proyectar desde la vela seleccionada: tomamos hasta idx+1
    windowEnd = Math.min(idx + 1, history.length);
  } else {
    // Proyectar la SIGUIENTE vela: tomamos hasta idx+2 (incluye la siguiente)
    windowEnd = Math.min(idx + 2, history.length);
  }

  // Necesitamos al menos 2 velas para analyzePattern
  const slice = effectiveHistory.slice(0, windowEnd);
  if (slice.length < 2) return null;

  const lastTwo = slice.slice(-2);

  // Reutilizar lógica de análisis vigente (extremeCondition y cmdState globales).
  // Esto es correcto: el modo sólo cambia el contexto de proyección, no el análisis.
  const crossResult = detectCrosses(false);
  const extremeCondition = crossResult.extremeCondition;

  return analyzePattern(lastTwo, null, extremeCondition);
}
...
function showHoverProj(idx, pinned) {
  ...
  const panel   = document.getElementById('hover-proj-panel');
  const badge   = document.getElementById('hover-proj-mode-badge');
  const infoEl  = document.getElementById('hover-proj-candle-info');
  const footer  = document.getElementById('hover-proj-footer');
  if (!panel) return;

  const proj = computeHoverProjection(idx);
  if (!proj) return;
  ...
  const candleColor = history[idx] === 'G' ? '🟢' : '🔴';
  const candleStr   = getCandleStrength(idx) || 2;
  const strLabel    = STRENGTH_CONFIG.LEVELS[candleStr]?.name || 'MEDIA';
  infoEl.textContent = `Vela #${idx+1} ${candleColor} · F${candleStr} ${strLabel}`;
  if (projectionMode === 'next') {
    const nextIdx = idx + 1;
    if (nextIdx < history.length) {
      const nc = history[nextIdx] === 'G' ? '🟢' : '🔴';
      infoEl.textContent += ` → Sig. #${nextIdx+1} ${nc}`;
    } else {
      infoEl.textContent += ' → Próxima vela';
    }
  }

  // Verde
  const gSeqEl = document.getElementById('hover-proj-g-seq');
  const gSigEl = document.getElementById('hover-proj-g-sig');
  const gConfEl= document.getElementById('hover-proj-g-conf');
  gSeqEl.textContent = proj.ifG.pat;
  gSigEl.textContent = proj.ifG.sig;
  gSigEl.className = 'hover-proj-sig ' + (proj.ifG.sig === 'COMPRA' ? 'sig-buy' : proj.ifG.sig === 'VENTA' ? 'sig-sell' : 'sig-wait');
  gConfEl.textContent = Math.round(proj.ifG.conf * 100) + '% · ' + proj.ifG.desc;

  // Rojo
  const rSeqEl = document.getElementById('hover-proj-r-seq');
  const rSigEl = document.getElementById('hover-proj-r-sig');
  const rConfEl= document.getElementById('hover-proj-r-conf');
  rSeqEl.textContent = proj.ifR.pat;
  rSigEl.textContent = proj.ifR.sig;
  rSigEl.className = 'hover-proj-sig ' + (proj.ifR.sig === 'VENTA' ? 'sig-sell' : proj.ifR.sig === 'COMPRA' ? 'sig-buy' : 'sig-wait');
  rConfEl.textContent = Math.round(proj.ifR.conf * 100) + '% · ' + proj.ifR.desc;

  // Footer
  const hint = document.getElementById('.hover-proj-pin-hint') || footer.querySelector('.hover-proj-pin-hint');
  if (hint) hint.textContent = pinned ? '📌 Fijado · Click fuera o ✕ para cerrar' : 'Click en vela para fijar · Hover para previsualizar';
...
function patchHistoryCandles() {
  ...
  // HOVER: mostrar proyección al pasar el mouse sobre una vela
  historyEl.addEventListener('mouseover', function(e) {
    if (hoverProjPinned) return;
    const candle = e.target.closest('.candle');
    if (!candle) return;
    const idx = parseInt(candle.dataset.hoverIdx ?? '-1');
    if (idx >= 0) showHoverProj(idx, false);
  });
  ...
  // CLICK (se delega; el onclick inline llama openEditPanel primero pero también queremos fijar panel)
  historyEl.addEventListener('click', function(e) {
    ...
    if (idx < 0) return;
    const panel = document.getElementById('hover-proj-panel');
    const currentPinIdx = panel ? parseInt(panel.dataset.idx ?? '-1') : -1;
    if (hoverProjPinned && currentPinIdx === idx) {
      closeHoverProj(); // toggle: click en la misma vela fijada la cierra
    } else {
...
// Cerrar panel fijado al hacer click fuera del historial
document.addEventListener('click', function(e) {
  if (!hoverProjPinned) return;
  const panel   = document.getElementById('hover-proj-panel');
  const historyEl = document.getElementById('history');
  if (!panel || !historyEl) return;
  if (!panel.contains(e.target) && !historyEl.contains(e.target)) {
    closeHoverProj();
  }
});
...
initFocusMode();
patchHistoryCandles();
console.log('🚀 Detector CMD iniciado (refactorizado). Modo proyección: activado.');
