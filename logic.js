...
function computeHoverProjection(idx) {
  ...
  var effectiveHist = history;
  if (manualTrapIndex !== null) {
    effectiveHist = history.filter(function(_, i) { return i !== manualTrapIndex; });
  }
  var windowEnd = projectionMode === 'current'
    ? Math.min(idx + 1, effectiveHist.length)
    : Math.min(idx + 2, effectiveHist.length);
  var slice = effectiveHist.slice(0, windowEnd);
  if (slice.length < 2) return null;
  var lastTwo = slice.slice(-2);
  var crossResult = detectCrosses(false);
  var extremeCond = crossResult.extremeCondition;
  return analyzePattern(lastTwo, null, extremeCond);
}
...
function positionHoverPanel(panel) {
  ...
  var offset = 16;
  var left = _hoverMouseX + offset;
  var top  = _hoverMouseY + offset;
  if (left + W > vw - 8) { left = _hoverMouseX - W - offset; }
  if (top  + H > vh - 8) { top  = _hoverMouseY - H - offset; }
  if (left < 8) { left = 8; }
  if (top  < 8) { top  = 8; }
...
function showHoverProj(idx, pinned) {
  ...
  if (!panel) return;
  var proj = computeHoverProjection(idx);
  if (!proj) return;
  ...
  var candleColor = history[idx] === 'G' ? 'VERDE' : 'ROJA';
  var candleStr   = getCandleStrength(idx) || 2;
  var strLabel    = (STRENGTH_CONFIG.LEVELS[candleStr] || {}).name || 'MEDIA';
  var infoText    = 'Vela #' + (idx + 1) + ' ' + candleColor + ' F' + candleStr + ' ' + strLabel;
  if (projectionMode === 'next') {
    var nextIdx = idx + 1;
    if (nextIdx < history.length) {
      infoText += ' \u2192 Sig. #' + (nextIdx + 1) + ' ' + (history[nextIdx] === 'G' ? 'VERDE' : 'ROJA');
    } else {
      infoText += ' \u2192 Pr\u00f3xima vela';
    }
  }
  infoEl.textContent = infoText;

  var gSigClass = proj.ifG.sig === 'COMPRA' ? 'sig-buy' : proj.ifG.sig === 'VENTA' ? 'sig-sell' : 'sig-wait';
  var rSigClass = proj.ifR.sig === 'VENTA'  ? 'sig-sell': proj.ifR.sig === 'COMPRA' ? 'sig-buy'  : 'sig-wait';
  ...
  gSigEl.className   = 'hover-proj-sig ' + gSigClass;
  ...
  rSigEl.className   = 'hover-proj-sig ' + rSigClass;
  document.getElementById('hover-proj-r-conf').textContent = Math.round(proj.ifR.conf * 100) + '% \u00b7 ' + proj.ifR.desc;

  if (hint) {
    hint.textContent = pinned
      ? '\ud83d\udccc Fijado \u00b7 Click en la misma vela o \u2715 para cerrar'
      : 'Click en vela para fijar \u00b7 Hover para previsualizar';
  }
...
function patchHistoryCandles() {
  ...
  // Cuando el mouse entra al panel flotante, cancelar el hide
  ...
  historyEl.addEventListener('mouseover', function(e) {
    ...
    while (t && t !== historyEl) {
      if (t.classList && t.classList.contains('candle')) {
        var idx = parseInt(t.getAttribute('data-hover-idx') || '-1');
  ...
  historyEl.addEventListener('mousemove', function(e) {
    cancelHide();
    if (hoverProjPinned) return;
    var panel = document.getElementById('hover-proj-panel');
    if (panel && panel.style.display !== 'none') {
...
// Cerrar panel cuando el mouse sale de él (con pequeño delay)
(function() {
  document.addEventListener('DOMContentLoaded', function() {
    ...
    panel.addEventListener('mouseleave', function() {
      if (hoverProjPinned) return;
      setTimeout(function() {
        // Solo ocultar si el mouse no volvió al historial
        if (!hoverProjPinned) {
...
initFocusMode();
patchHistoryCandles();
console.log('Detector CMD iniciado. Modo proyeccion activo.');
